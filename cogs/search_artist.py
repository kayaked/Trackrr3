    ...
    async def search_artist(self, ctx, *, artist_name=None):
        ...
        if not artist_name:
            ...
            embed = discord.Embed(
                ...
                description=f'`{cmdprefix}{ctx.invoked_with} <artist name>`'
            )
            embed.set_image(
                url='https://i.gyazo.com/7d33394b56834ca32dc4c5db068806fe.png'
            )
            embed.set_footer(
        ...
        async with ctx.channel.typing():
            try:
                ...
                embed = discord.Embed(
                    title=f'Trackrr',
                    description=f'No results found for `{artist_name}`!'
                )
            embed.set_footer(
                text="Trackrr Music Search | Data pulled from Apple Music", 
                icon_url="https://media.discordapp.net/attachments/452763485743349761/452763575878942720/TrackrrLogo.png"
            )

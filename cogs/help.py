...
class Help:
    ...
    async def _help(self, ctx, page='1'):
        ...
        if page=='1':
            ...
            embed.add_field(name=f'`{cmdprefix}upload_song` `<optional service>` `[MP3 ATTACHMENT]`', value=f'Attach an MP3 file to your message and receive info about the song you uploaded (optionally search it on a music service if you provide one).')
            embed.add_field(name=f'`{cmdprefix}prefs`', value=f'Preference menu for Trackrr. Has options for the current guild and user, such as the guild\'s current prefix.\n\nDo `{cmdprefix}help 2` to view the next page of commands.')
        elif page=='2':
            embed.add_field(name=f'`{cmdprefix}lyrics` `<*song name>`', value=f'Scans lyrics from Genius for a song.')

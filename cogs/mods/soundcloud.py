...
async def search_album(album_name):
    ...
    # Taken from Yak's SoundCloud cog for LilYak2K
    ...
    async with aiohttp.ClientSession() as session:
        if album_name.startswith("id:") and album_name[3:].strip().isdigit():
            ...
            params = {'client_id': SoundCloudAPI.TOKEN, 'q': album_name, 'limit': '1'}
...
async def search_song(song_name):
    """ Searches an song per its name on SoundCloud. """
    async with aiohttp.ClientSession() as session:
        if song_name.startswith("id:") and song_name[3:].strip().isdigit():
            params = {'client_id': SoundCloudAPI.TOKEN, 'limit': '1'}
            async with session.get(f"{SoundCloudAPI.BASE}/track/{song_name[3:].strip()}", params=params) as resp:
                try:
                    form = await resp.json()
                except IndexError:
                    raise NotFound
            return SoundCloudSong(form)
        else:
            params = {'client_id': SoundCloudAPI.TOKEN, 'q': song_name, 'limit': '1'}
            async with session.get(f"{SoundCloudAPI.BASE}/search/tracks", params=params) as resp:
                try:
                    form = await resp.json()
                    form = form['collection'][0]
                except IndexError:
...
class SoundCloudAlbum(Album):
    ...
    def __init__(self, data: dict):
        ...
        self.cover_url = data.get('artwork_url').replace('large', 't500x500') if data.get('artwork_url') else data.get('track_list_raw', [])[0].get('artwork_url', 'https://github.com/exofeel/Trackrr/blob/master/assets/UnknownCoverArt.png?raw=true').replace('large', 't500x500') if data.get('tracks') else ""
...
class SoundCloudSong(Song):
    ...
    def __init__(self, data: dict):
        ...
        self.artist = data.get('user', {}).get('username', 'N/A')
        self.link = data.get('permalink_url', 'https://soundcloud.com/')
        self.track_album = 'N/A'
        self.cover_url = data.get('artwork_url').replace('large', 't500x500') if data.get('artwork_url') else data.get('user', {}).get('artwork_url', 'https://github.com/exofeel/Trackrr/blob/master/assets/UnknownCoverArt.png?raw=true').replace('large', 't500x500')
        self.release_date = datetime.strptime(data.get('created_at', '1970-01-01T00:00:00Z'), "%Y-%m-%dT%H:%M:%SZ")
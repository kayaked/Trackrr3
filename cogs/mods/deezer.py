...
async def search_song(song_name):
    """ Searches a song by name on Deezer. """
    params = {
        'q':song_name,
        'index':0,
        'limit':1,
    ...
    # cool of Deezer to not require a key, just wanna throw that 1 out there
    async with aiohttp.ClientSession() as session:
        async with session.get(DeezerAPI.BASE + '/search/track/', params=params) as resp:
            response = await resp.json()
    ...
    async with aiohttp.ClientSession() as session:
        # Example: http://api.deezer.com/album/65434762
        async with session.get(DeezerAPI.BASE + f'/track/{results.get("id", 0)}', params=params) as resp:
            response = await resp.json()
    responsetr = response.get('tracks', {}).get('data', [])
    results['contributors'] = response.get('contributors', [])
    results['release_date'] = response.get('release_date', '1970-01-01')
    return DeezerSong(results)
...
class DeezerSong(Song):
    ...
    def __init__(self, data:dict):
        ...
        self.cover_url = data.get('album', {}).get('cover_xl') if data.get('album', {}).get('cover_xl') else 'https://github.com/exofeel/Trackrr/blob/master/assets/UnknownCoverArt.png?raw=true'
        self.release_date = datetime.strptime(data.get('release_date', '1970-01-01'), '%Y-%m-%d')